...
### 1. Frontmatter (YAML header)
  ...
  **RSS Feed Distribution:**
  ...
  ### 4. Required Components and Imports
  ...
  Wrap all content in BlogWrapper:
  ```jsx
  <BlogWrapper>
    [Your content here]
  </BlogWrapper>
  ...
  **REQUIREMENT: Include at least one graphic per blog post**
  ...
  - Store images locally in the blog post directory
  - Use descriptive filenames (e.g., `meshery-feature-overview.png`)
  ...
  - Reference in content:
    ```jsx
    <img src={myImage} className="image-center" alt="descriptive alt text" />
    ```
  - Available CSS classes:
    - `image-left` / `image-right` - floated with shadow
    - `image-left-no-shadow` / `image-right-no-shadow` - floated without shadow
    - `image-center` / `image-center-shadow` - centered
  ...
  ### 6. BlockQuote Components for Emphasis
  ...
  **REQUIREMENT: Use BlockQuote or BlockQuoteAlt to emphasize central points as appropriate**
  ...
  ### 7. Call-to-Action (CTA) Components
  ...
  **REQUIREMENT: Include at least one CTA per blog post**
  ...
  Other CTA options:
  - `CTA_Bottom` - Bottom banner style
  - `CTA_ImageOnly` - Image-focused CTA
  ...
  **REQUIREMENT: Include as many intra-site links as possible**
  ...
  ### 9. Blog Styling with CSS Classes

  Available specialized CSS classes from Blog.style.js:

  **Intro sections:**
  ```jsx
  <div class="intro">
    <p>Introductory text with special styling</p>
  </div>
  ```

  **Note/Callout sections:**
  ```jsx
  <div class="note">
    <a href="url">Call to action text</a>
  </div>
  ```

  **Tip boxes (floated right):**
  ```jsx
  <div class="tip">
    <h3>Pro Tip</h3>
  ...
  **Keywords:**
  ...
  - Common keywords: cloud native, Kubernetes, service mesh, Meshery, Kanvas, 
    DevOps, platform engineering, infrastructure as code, GitOps, observability

  ### 11. Resources Collection Consideration
  ...
  **Community Focus:**
  ...
  ### 13. Image Generation and Sourcing

  **For image generation:**
  - Create placeholder references for images that need to be generated
  - Suggest image types and content in comments
  - Note: "// TODO: Generate image showing [description]"

  **For community shared drive:**
  - Reference images from Layer5 brand assets when appropriate
  - Use existing icons and logos from:
    - `/src/assets/images/meshery/`
    - `/src/assets/images/kanvas/`
    - `/src/assets/images/layer5/`
  ...
  <div class="intro">
    <p>
      Design patterns have revolutionized software development, and now they're transforming 
      cloud native infrastructure management through <Link to="/cloud-native-management/meshery">Meshery</Link>.
    </p>
  </div>
  ...
  <img src={patternImage} className="image-center" alt="Meshery design patterns overview" />
  ...
  Main file: `post.mdx` or `index.mdx`
  Images: Store in the same directory

  Example: `/src/collections/blog/2024/11-20-getting-started-with-meshery-patterns/post.mdx`
  ...
  - [ ] At least one graphic is included
  ...
  - [ ] Proper imports and BlogWrapper
